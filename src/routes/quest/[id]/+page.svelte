<script lang="ts">
	...
	// Die ID aus der URL ziehen
	let animalId = $derived($page.params.id);

	// Wir suchen das Tier und den dazugehörigen Lebensraum (Biom) elegant heraus
	let animalData = $derived(
		database.biomes
			.flatMap(b => b.animals.map(a => ({ animal: a, biome: b })))
			.find(x => x.animal.id === animalId)
	);

	// Wir filtern nur die Fragen für dieses Tier und mischen sie
	let animalQuests = $derived(animalData ? animalData.animal.quests.toSorted(() => Math.random() - 0.5) : []);
	
	// Wir suchen max. 4 andere Tiere aus demselben Biom heraus, um sie unten als Empfehlung anzuzeigen
	let relatedAnimals = $derived(animalData ? animalData.biome.animals.filter(a => a.id !== animalId).slice(0, 4) : []);
	...
	// Wenn der User unten auf ein "Ähnliches Tier" klickt, ändert sich nur die URL. 
	// Dieser Effekt resettet das Quiz, sobald das passiert.
	$effect(() => {
		animalId; // Wir sagen Svelte, dass dieser Effekt auf animalId reagieren soll
		currentIndex = 0;
		selectedAnswer = null;
		isSubmitted = false;
		window.scrollTo(0, 0);
...
<main class="min-h-screen bg-background text-on-background pb-32 px-4 sm:px-6 md:px-12 max-w-2xl mx-auto pt-24">
	...
	{:else}
		...
		<a href="/quest" class="inline-flex items-center gap-2 text-on-surface-variant hover:text-primary mb-6 transition-colors p-2 -ml-2 rounded-lg active:scale-95">
			<span class="material-symbols-outlined">arrow_back</span>
			<span class="font-bold text-sm tracking-wide">Zurück zur Auswahl</span>
		</a>
		...
		<div class="relative w-full aspect-[4/3] rounded-3xl overflow-hidden mb-6 border-2 border-outline-variant/20 shadow-2xl">
			{#key currentQuestion.image}
				<img src={currentQuestion.image} alt="Quest Tier" class="w-full h-full object-cover" in:fade={{ duration: 400 }} />
			{/key}
		...
		<div class="flex flex-col gap-3 mb-8">
			{#each currentQuestion.options as option}
				<button 
					...
					{isSubmitted ? (option.isCorrect ? 'bg-primary-container/40 border-primary text-primary' : (selectedAnswer === option.id ? 'bg-error-container/40 border-error text-error' : 'bg-surface-container-lowest border-outline-variant/10 text-on-surface opacity-40 grayscale-[50%]')) : 'bg-surface-container-low border-outline-variant/15 text-on-surface hover:border-primary/50 hover:bg-surface-container shadow-sm'}" 
				>
					<div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold font-label shrink-0 transition-colors {isSubmitted ? (option.isCorrect ? 'bg-primary text-on-primary' : (selectedAnswer === option.id ? 'bg-error text-on-error' : 'bg-surface-container-highest text-on-surface-variant')) : 'bg-surface-container-highest text-on-surface-variant'}">
		...
		{#if isSubmitted}
			<div transition:slide={{ duration: 400 }}>
				<div class="pb-8 pt-4">
					<div class="bg-surface-container-lowest rounded-3xl p-5 sm:p-6 shadow-xl border border-outline-variant/15 relative overflow-hidden">
						...
						<h3 class="font-headline font-bold text-xl text-tertiary mb-6 flex items-center gap-2">
							<span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">lightbulb</span> Wusstest du schon?
						</h3>
				...
				<button onclick={nextQuestion} class="w-full bg-gradient-to-br from-primary to-primary-container text-on-primary py-5 rounded-2xl font-bold text-lg shadow-lg shadow-primary/20 active:scale-95 transition-all flex justify-center items-center gap-3 border border-primary-fixed/30 mb-12">
					{currentIndex < animalQuests.length - 1 ? 'Nächste Frage' : 'Zurück zur Auswahl'}
					<span class="material-symbols-outlined text-2xl font-bold">{currentIndex < animalQuests.length - 1 ? 'arrow_forward' : 'list'}</span>
				</button>
			</div>
		{/if}

		<div class="mt-12 pt-10 border-t border-outline-variant/20">
			<h3 class="text-xl font-bold text-on-surface mb-6 tracking-tight">Weitere Arten aus: {animalData?.biome.name}</h3>
			
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
				{#each relatedAnimals as related}
					<a href="/quest/{related.id}" class="bg-surface-container-low rounded-xl overflow-hidden border border-outline-variant/15 hover:border-primary/50 transition-all active:scale-95 group block">
						<div class="aspect-square w-full relative">
							<img src={related.lexicon.image_main} alt={related.name} class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
							
							{#if playerState.collectedSpecies.includes(related.id)}
						...
						<div class="p-3 text-center bg-surface-container">
							<h4 class="font-bold text-on-surface text-xs truncate">{related.name}</h4>
						</div>
					</a>
				{/each}
			</div>
